...
def apply_Fe3_fraction(f_Fe3,composition_wt):
...
modified_df = apply_Fe3_fraction(f_Fe3,compositions.iloc[0])
...
for sample in compositions.index:
    ...
    corrected_composition = apply_Fe3_fraction(f_Fe3_values[sample],compositions.loc[sample])
...
#+BEGIN_SRC python :session py
# Set all SiO2 wt% to 44.
compositions["SiO2"] = 44
# Renormalize
compositions = compositions.div(compositions.sum(axis=1)/100,axis=0)
#+END_SRC

#+RESULTS:

* Serpentinisation Path End Corrections
Furthermore, since this oxidation is likely to have arisen from being near the (oxidizing) earth's surface (i.e. is a recent effect), the final composition in the PEM (i.e. the final serpentinite composition) will also be Fe-oxidation corrected: \cite{Eberhard2023} finds that antigorite serpentinite has a Fe3+/FeTot is 0.4, which will be assumed true for the theoretical, unweathered serpentinite of Santa Catalina.
